...
| The PASSIVE-SSL layer — historical **cert → IP**, the ORIGIN-RECOVERY direction and the partner of passive DNS. Every other TLS source reads the cert a host presents NOW (live handshake) or the names a CA logged AT ISSUANCE (crt.sh/Censys); this answers *which addresses have served this exact leaf certificate over time*, so the box an operator served it on before hiding behind Cloudflare is still recoverable. pDNS = historical name→IP, pSSL = historical cert→IP; an address in BOTH is `origin_corroborated`, the strongest lead the pair gives. Same free CIRCL account as pDNS, and **sha1-keyed** (Censys is sha256) — so `fetch_tls_cert` now records `fingerprint_sha1` from the DER bytes it already read, costing no extra connection to the target. 🚫 **The base rate matters more here than anywhere**: a shared CDN cert is served by thousands of unrelated IPs (a Cloudflare edge cert measured at **915**), so past `max_ips_per_cert`, or on a CDN subject, it is INFRASTRUCTURE and `clusterable` is false — trust that field, never re-derive it. Coverage is Europe-weighted: an empty answer is **absence of RECORD**, never "served no certificate" | `WebPivot/tools/wp_pssl.py` (`passive_ssl` tool; auto in `pivot_extract`, `--no-pssl`) + `WebPivot/references/pssl.json` (`clustering_policy` = the rail) + `wp_analyze` enrichment (`pssl` + `origin_corroborated`) + `tests/test_pssl.py` (in the eval gate) |
| The MISCONFIG-TRIAGE layer — the tell that a box is OPERATOR-RUN AND SLOPPY rather than hardened/CDN, read PASSIVELY from a banner FOFA/Shodan/Censys already indexed (no packet sent). Two signals. **Internal-IP leak**: an RFC1918/loopback/link-local address surfacing in a public result's `ip`/`host`/**banner** (a redirect `Location`, a self-signed cert SAN, a config echo) = a DUAL-HOMED box exposing its internal topology → pivot the leaked host/redirect for the real origin or internal map; a CDN edge never leaks this. **Anonymous FTP**: a `230` anon-login banner is a HIGH-value lead — the kit, uploaded VICTIM logs, builder configs, sometimes the operator's own box. 🚫 **FLAGS ONLY — the tool never auto-connects**: opening the FTP is active + attributable (it tips the operator) and victim data has handling/legal implications, so a human decides. Base-rate safe: the row's OWN public IP is never a leak; CGNAT (`100.64/10`) is off by default. IPPivot FUSES FOFA+Shodan+Censys for the view this reads over — combine all three to widen the historical window | `WebPivot/tools/wp_recon.py` (`scan_misconfig`) + `wp_ippivot.fofa_ip`/`build_ip_result` (the `ip:misconfig` pivot; auto in bare-IP `pivot_extract`) + `WebPivot/references/misconfig_signals.json` (`anon_ftp_markers`/`ftp_protocol_markers`/`leak_classes`) + `tests/test_misconfig.py` (in the eval gate) |
...
| The INTAKE record ON THE HARNESS side — the same six classes, but for the front-ends that have NOBODY TO ASK (`intel.py open`, the SDK driver, MCP, a batch). Given once as CLI flags / `--scope` file / the `case_scope` MCP tool, persisted to `cases/<case>/scope.json`, rendered into EVERY phase prompt as `{{scope}}` on every later round and resume. Three effects: POSTURE is **enforced, not suggested** — a no-touch class (or `--no-direct-contact`) derives `hostile`, which the `audit.py` gate turns into a hard denial of outbound collection, while `passive_first` deliberately does NOT (it is an ordering instruction; conflating the two would silently make every unscoped run a no-fetch run). OWNERSHIP reaches the JUDGE as well as the collector — the collector labelling a victim's artifacts correctly is no use if the correlator still clusters on them. THE CLAIM travels with its source and its falsifier and is ANSWERED: `Assessment` gained `premise` + `premise_verdict`, defaulting to `inconclusive` so an omission can never read as a claim confirmed. A defaulted value is never rendered back to the model as an answer (`stated_fields`) — a defaulted `time_window` reading as an incident date is the same anchoring failure one level down | `harness/case_scope.py` (+ `orchestrator._scope` / `_scope_from_argv`, `harness/prompts/*.md` `{{scope}}`, `harness/schemas.py`, `harness/render.py`) + `WebPivot/references/intake.json` (shared owner) + `tests/test_case_scope.py` (in the eval gate) |
| The DEBUG DASHBOARD — a local, stdlib-only web UI for "where did the tokens go" and "what went wrong", over BOTH front-ends. Reads five append-only sources and joins them; instruments nothing, writes nothing, sends nothing: Claude Code session transcripts (`~/.claude/projects/<repo>/*.jsonl` — exact per-turn input/output/cache-read/cache-write incl. the 1h-vs-5m split, model, effort, tool calls, stop_reason, subagent turns), the gate ledger, the API-credit ledger, `run_cost.jsonl`, and the pinned prompt files. Panels: findings · **trace** · tokens & cache · **prompt surface** (what the window holds before turn 1 — the harness pins whole SKILL.md bodies, so a paragraph added to a skill is paid on every phase of every case) · tool calls & denials · cost vs credits. **TRACE is the "what did it DO" panel** — one session replayed in order: the prompt in, the pinned context it carried (folded), every tool call with its ARGUMENTS and the RAW RESULT that came back, the reply out, each turn's model/effort/tokens/cost, and a run-flow ribbon that puts the whole session on one line and jumps to any step. Display cuts are loud (head + tail, the exact dropped count, and a `show full` that re-reads the transcript) because a shortened result reads as a tool that found little; a base64 screenshot is described, never inlined; a gate badge appears only on an EXACT (tool, arguments) ledger match, so no badge means no record rather than an allowed call. **One API response spans several transcript records** (thinking, then one per `tool_use`) each repeating the SAME `usage` — every reader here bills once per `requestId`, or a tool-heavy turn is counted 2–4×; `tools/cost_report.py` does not yet do this. **Two kinds of number, never mixed** — `usage`-derived counts are EXACT, anything measured off a file is an ESTIMATE and renders differently; dollars are list-price estimates and `tools/cost_report.py` owns the price table (imported, never restated). Honest about itself: a bounded scan says it is bounded, an absent ledger is ABSENCE OF RECORD, an unpriced model becomes a finding because its tokens make every total an under-estimate. **Loopback only** — it renders case data with no auth, so a non-loopback bind is refused and the refusal names the SSH tunnel | `harness/dashboard/serve.py` + `collect.py` + `static/` + `harness/references/dashboard.json` (thresholds, findings rules + their `why`, scan bounds, prompt-surface list) + `tests/test_dashboard.py` (in the eval gate) |
