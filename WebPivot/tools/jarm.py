"""JARM active TLS-server fingerprinting — vendored, stdlib-only.

The ALGORITHM functions below are the canonical Salesforce JARM implementation,
vendored verbatim so the 62-hex output byte-matches the JARM ecosystem
(Shodan `ssl.jarm:`, Censys `services.jarm.fingerprint`, ZoomEye `jarm=`).
Only the CLI / file-output / SOCKS scaffolding was dropped and a clean
`jarm_fingerprint(host, port, timeout)` entry point added.

    # Version 1.0 (November 2020) — John Althouse, Andrew Smart, RJ Nunaly,
    # Mike Brady. Python port by Caleb Yu.
    # Copyright (c) 2020, salesforce.com, inc. All rights reserved.
    # Licensed under the BSD 3-Clause license.
    # https://github.com/salesforce/jarm  ·  https://opensource.org/licenses/BSD-3-Clause

JARM sends 10 deliberately-varied TLS Client Hellos and hashes how the server
responds — a fingerprint of the server's TLS *stack + config* that survives
domain/branding changes. Same JARM across hosts ⇒ candidate same deployment.
This is an ACTIVE probe (10 handshakes); the caller gates it to the primary
live host and suppresses it under --proxy (raw socket can't use an HTTP proxy).
"""
import codecs
import socket
import struct
...
# ---- stdlib entry point (replaces the reference CLI / send_packet globals) ----

# [version, cipher_list, cipher_order, GREASE, ALPN, supported-versions, ext-order]
_QUEUE = [
...
def _send_packet(packet, host, port, timeout):
...
def jarm_fingerprint(host, port=443, timeout=15):
    """Return {target, ip, jarm, empty} for host:port. `jarm` is the 62-hex JARM
    ('0'*62 / empty=True means no usable TLS server hello was obtained)."""
    jarm = ""
    ip = None
    for i, spec in enumerate(_QUEUE):
        details = [host, port] + spec
        server_hello, ip = _send_packet(packet_building(details), host, port, timeout)
        if server_hello == "TIMEOUT":
            jarm = _EMPTY
            break
        jarm += read_packet(server_hello, details)
        if i != len(_QUEUE) - 1:
            jarm += ","
    result = jarm_hash(jarm)
    return {"target": f"{host}:{port}", "ip": ip, "jarm": result, "empty": result == "0" * 62}
